...
echo "<h1>Vérification de la table classement</h1>";
...
try {
    ...
    if (!$tableExists) {
        die("<div style='color: red;'>La table 'classement' n'existe pas dans la base de données.</div>");
    }
    
    echo "<div style='color: green;'>✓ La table 'classement' existe.</div>";
    
    // 2. Vérifier la structure de la table classement
    echo "<h2>2. Structure de la table classement</h2>";
    ...
    if (count($missingColumns) > 0) {
        ...
        echo "<strong>Colonnes manquantes dans la table 'classement':</strong><br>";
        echo "- " . implode("<br>- ", array_keys($missingColumns));
        echo "</div>";
    } else {
        echo "<div style='color: green; margin-top: 10px;'>";
        echo "✓ Toutes les colonnes requises sont présentes dans la table 'classement'.";
        echo "</div>";
    }
    ...
    if (count($primaryKeys) > 0) {
        ...
        $expectedPrimaryKeys = ['saison', 'competition', 'poule_id', 'nom_equipe'];
        ...
        echo "<div style='color: red;'>Aucune clé primaire trouvée pour la table 'classement'.</div>";
    }
    
    // 4. Vérifier les index
    echo "<h2>4. Index de la table classement</h2>";
    $stmt = $pdo->query("
        SHOW INDEX FROM classement
    ");
    ...
    if (count($indexes) > 0) {
        ...
        echo "<p>Aucun index trouvé pour la table 'classement'.</p>";
    }
    
    // 5. Vérifier les données existantes
    echo "<h2>5. Données existantes</h2>";
    $stmt = $pdo->query("SELECT COUNT(*) as count FROM classement");
